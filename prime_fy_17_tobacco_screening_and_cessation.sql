...
WHERE 
     ...
     AND patient_encounter.billable_timestamp <='2018-06-30 23:59.999'
      And billable_ind = 'y'
	 AND CAST (tobacco_usage_.enc_date AS datetime) >= '2016-07-01'
	 AND CAST (tobacco_usage_.enc_date AS datetime) <= '2018-06-30'
...
WHERE 
	 ...
	 AND DATEDIFF(YEAR,person.date_of_birth, tobacco_usage_.enc_date)>= 18 
	 AND NOT (person.first_name like '%test' or person.last_name like '%test')
	 And tobacco_cessation_discuss_.create_timestamp >='2016-07-01'
 GROUP BY  patient.med_rec_nbr
 HAVING COUNT(patient_encounter.billable_timestamp) >= 2 
