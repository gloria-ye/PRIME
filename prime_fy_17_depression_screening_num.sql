...
/* INNER JOIN patient_diagnosis on patient_diagnosis.person_id = depression_PHQ_9_.person_id*/
 WHERE 
	...
	AND CAST (depression_PHQ_9_.encDate AS datetime) >= '2017-07-01'
	AND CAST (depression_PHQ_9_.encDate AS datetime) <= '2018-06-30'
	AND patient_medication.start_date >= '20170701'
	AND patient_medication.start_date <= '20180630'
	AND DATEDIFF(YEAR,person.date_of_birth, depression_PHQ_9_.encDate)>= 18
	AND billable_ind = 'Y'
	AND (patient_.full_name not like '%test%' or patient_.full_name not like '%MCHC%')
...
INNER JOIN mch_depression_PHQ_9_83_  on mch_depression_PHQ_9_83_.enc_id = depression_PHQ_9_.enc_id
 ...
 HAVING (COUNT(patient_encounter.billable_timestamp) >= 2 AND NOT patient_diagnosis.description like 'bipolar' AND (patient_diagnosis.create_timestamp <= CAST (MAX (depression_PHQ_9_.encDate)as datetime) or (patient_diagnosis.create_timestamp >= '2017-07-01' and patient_diagnosis.create_timestamp <= '2018-06-30')))
